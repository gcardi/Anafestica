...
> *Note: This tour uses the Windows Registry backend (`TConfigRegistrySingleton`), but the library supports additional storage backends — JSON, BSON, XML, and INI files — with the same programming model. To switch backends, replace the singleton type alias with `TConfigJSONSingleton`, `TConfigBSONSingleton`, `TConfigXMLSingleton`, or `TConfigINIFileSingleton` from the corresponding singleton header. See [Library Documentation](LIBRARY_DOCUMENTATION.md) for details.*
