...
> *Note: This tour uses the Windows Registry backend (`TConfigRegistrySingleton`), but the library supports additional storage backends — JSON, BSON, YAML, XML, and INI files — with the same programming model. To switch backends, replace the singleton type alias with `TConfigJSONSingleton`, `TConfigBSONSingleton`, `TConfigYAMLSingleton`, `TConfigXMLSingleton`, or `TConfigINIFileSingleton` from the corresponding singleton header. YAML support requires the external header-only fkYAML library to be registered in RAD Studio's include search path for the Clang-based compiler platforms you use. See [Library Documentation](LIBRARY_DOCUMENTATION.md) for details.*
