...
Anafestica uses `boost::variant` instead of `std::variant` due to an unresolved bug in C++Builder's standard library that prevents direct assignment of values to `std::variant` (see https://quality.embarcadero.com/browse/RSP-27418 on the Embarcadero Quality Portal). With the modern `bcc64x` compiler (Clang 20), you must define `ANAFESTICA_USE_STD_VARIANT` as a project-wide preprocessor definition; otherwise, compilation fails because of incompatibilities with `boost::variant` in this toolchain.
