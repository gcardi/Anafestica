...
#ifndef CfgJSONCryptSingletonH
#define CfgJSONCryptSingletonH
...
static constexpr LPCTSTR ConfigFileExtension = _D( ".jsonc" );
                    ...
                    Crypt::TOptions Options = Crypt::TOptions::Default() )
{
    static auto Cfg = [FileName, Options]() -> TConfig {
        ...
        if ( !TFile::Exists( Dest ) ) {
            ...
            {
                return TConfig::Migrate( *Source, Dest, false, true, false, Options );
            }
        }
        return TConfig( Dest, false, true, false, false, Options );
    }();
    return Cfg;
}
//---------------------------------------------------------------------------
...
public:
    static Anafestica::TConfig& GetConfig() {
        return Anafestica::JSONCrypt::GetConfigSingleton();
    }
};
...
using TConfigJSONCryptSingleton = JSONCrypt::TConfigSingleton;
